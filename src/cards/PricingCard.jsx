...
const PricingCard = ({ rates,title, price, description,IconColor, features, buttonLabel, buttonColor, Color , BgColor , TextColor}) => {
  return (
    <div className={`p-3 flex flex-col border border-gray-400 justify-between items-center rounded-xl bg-${BgColor}`}>
  <div className="flex flex-col">
    <div className={`py-3 font-medium text-${TextColor} text-xl`}>{title}</div>
  ...
  <div>
    <button className={`font-semibold text-base text-${Color} rounded-xl p-5 w-full bg-${buttonColor} hover:bg-yellow-700`}>
      {buttonLabel}
    </button>
  </div>
</div>
